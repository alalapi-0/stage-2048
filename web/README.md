...
## 规则编辑器
<!-- 规则编辑器入口说明 -->
- 设置面板新增「打开规则编辑器」链接，指向 `rules.html`，可在新标签页使用表单方式管理完整 SETTINGS JSON。
<!-- 规则编辑器功能说明 -->
- 编辑器包含基础字段、LEVELS、随机权重与插件多选，可导出下载、从 `.json` 文件导入以及写入本地存储。
<!-- 规则编辑器校验说明 -->
- 表单会校验尺寸范围与概率和，并在应用时提示新规则可能影响当前进度；保存后回到主页面刷新即可加载新配置。
...
## 关卡包
- 设置面板新增“导入关卡包（JSON）”文件输入与“清除关卡包”按钮，分别写入或移除 `localStorage['stage2048.levelpack.v1']`。
- 导入成功会在状态栏与提示区域显示包名与关卡数，当前对局会按包参数重新创建，进入下一关也会优先读取包内配置。
- 点击“清除关卡包”后立即恢复默认的尺寸递增推进，提示区域同步更新为“未加载关卡包”。
- `packs/sample-pack.json` 提供可直接导入的示例文件，完整格式与校验规则请参阅《[关卡包使用指南](../docs/LEVEL_PACKS_GUIDE.md)》。
